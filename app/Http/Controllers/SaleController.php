...
{
    private const SALES_DASHBOARD_STATS_TTL_SECONDS = 90;
    ...
    public function index(IndexSaleRequest $request): View
    {
        ...
        ksort($filtersForKey);
        $filterHash = hash('sha256', json_encode($filtersForKey, JSON_THROW_ON_ERROR));
        $statsCacheKey = PanelCacheKeys::salesDashboard((int) $store->id, $filterHash);

        $dashboard = Cache::remember(
            $statsCacheKey,
            self::SALES_DASHBOARD_STATS_TTL_SECONDS,
