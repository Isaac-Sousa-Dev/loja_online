            ...
            <div class="h-32 md:h-80 w-[100%] md:w-full md:rounded-b-[80px] flex items-center justify-center">
                ...
                @if($bannerStore != null)
                    <div class="object-cover h-full w-full rounded-b-[100px] border-4 border-t-0 border-white">
                        <img src="{{$bannerStore}}" alt="" class="object-cover object-center h-full w-full rounded-b-[100px]" width="80%" height="100%">
                    </div>
                @else
                    <div class="object-cover bg-blue-800 h-full w-full rounded-b-[100px] border-4 border-t-0 border-white"></div>
                @endif

                @if ($logoStore != null)
                    <div class=" bg-slate-200 md:ml-[12%] mt-32 w-32 h-32 rounded-full flex justify-center items-center absolute" style="border: 4px solid white; overflow: hidden;">
                        <img src="{{ $logoStore }}" class="object-cover object-center w-full h-full" alt="Logo">
                    </div>
                @else
                    <div class=" bg-slate-200 md:ml-[12%] mt-32 min-[414px]:h-[20%] w-[35%] min-[390px]:h-[16%] min-[414px]:w-[43%] rounded-full flex justify-center items-center absolute" style="border: 4px solid white; overflow: hidden;">
                        <img src="https://s3-sa-east-1.amazonaws.com/projetos-artes/fullsize%2F2020%2F12%2F12%2F09%2FLogo-272799_34049_091615787_599428780.jpg" class="object-cover" alt="">
                    </div>
                @endif
            </div>

            <div class="min-[414px]:-mt-1 min-[412px]:-mt-2 min-[414px]:h-52 mx-2 flex justify-center flex-col rounded-bg-lg">
        ...
        <div>
            <div class="px-3 mt-4 flex justify-between h-12">
                ...
                <div class="w-5/6 space-x-2 flex justify-end items-center relative">
                    <input type="text" class="w-full h-11 rounded-full border border-gray-400 pl-7" placeholder="Pesquise aqui...">
                    <ion-icon name="search-outline" class="search-icon text-2xl absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400"></ion-icon>
                </div>
            </div>
            <div id="div-categorias" class="mt-4 py-2 px-3 flex space-x-3 w-full overflow-x-auto">
                <div class="flex space-x-3">
                    ...
                    <div style="letter-spacing: 1px" class="font-bold text-white w-24 justify-center rounded-full p-1 flex div-categoria bg-blue-600 cursor-pointer" data-categoryId="{{ $category['id'] }}">
                        {{ $category['name'] }}
                    </div>
                    @endforeach
                </div>
    ...
    <script>
        ...
        function formatPrice(price) {
            // return price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
             ...
             // Formata o preço para o padrão brasileiro com o símbolo "R$"
                        ...
                        cards += `
                            <div id="car-product" class="class_card_product card p-1 w-full shadow-sm pb-4 bg-white mt-3 ">
                                <a href="/orders/${partnerLink}/product/${product.id}">
                                    <img class="rounded-t-lg w-full h-32 object-cover object-center" src="/storage/${product.image_main}" alt="" >
                        ...
                        data.forEach(product => {
                            ...
                            cards += `
                                <div id="car-product" class="class_card_product card p-1 w-[50%] pb-4 bg-white mt-3">
                                    <a href="/orders/${partnerLink}/product/${product.id}">
                                        <img class="rounded-lg w-full h-32 object-cover" src="/storage/${product.image_main}" alt="" >
                                        <div class="h-1/2 flex justify-between flex-col">
                                            <div class="mt-2 mx-1">
                                                <div>
