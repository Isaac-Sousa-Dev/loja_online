<div class="">
    <div class="h-12 flex items-center py-5 justify-between shadow-sm fixed w-full bg-white top-0 z-10">
        <div class="text-lg flex gap-1 items-center font-bold text-gray-700 ml-5">
            @if ($logoStore != null)
                <div class=" bg-slate-200 rounded-full w-8 h-8 flex justify-center items-center" style="border: 4px solid white; overflow: hidden;">
    ...
    <div class="bg-white fixed bottom-0 w-full p-3 flex justify-center z-10">
        <button class="btn-send-message-pdp bg-blue-700 px-4 py-3 w-full rounded-lg font-bold text-white">Enviar Mensagem</button>
    </div>

    {{-- Modal send Message --}}
    <div class="modal-send-message hidden fixed inset-0 bg-black bg-opacity-50 z-50">
        <div class="modal-send w-11/12 md:w-1/2 bg-white p-4 rounded-lg absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            ...
            <div class="mt-4">
                ...
                <input type="text" id="inputModalPdpName" placeholder="Seu nome" class="w-full border border-gray-300 rounded-lg p-2 mt-1">
                <span id="spanModalPdpName" class="hidden">
                    <div class="gap-1 items-center text-sm text-red-500 flex">
                        <svg class="w-4 h-4 text-red-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            ...
            <div class="mt-4">
                ...
                <input id="inputModalPdpPhone" type="text" placeholder="Seu telefone" class="phone-mask w-full border border-gray-300 rounded-lg p-2 mt-1">
            ...
            <div class="mt-3">
                ...
                <div class="flex gap-3">
                    <div class="mt-1 flex items-center gap-2">
                        ...
                        <label for="replacement" >Dar veículo na troca</label>
                    </div>
                    <div class="mt-1 flex items-center gap-2">
...
Olá, tenho interesse no veículo {{$product->name}}. Gostaria de receber mais informações sobre o veículo. Poderia entrar em contato?
            ...
            <div class="mt-4">
                <button id="btnSendSolicitationPdp" class="bg-blue-700 text-white font-bold px-4 py-2 rounded-lg w-full">Enviar</button>
            </div>
        </div>
    </div>
</div>
        ...
        if (!validateEmptyFields()) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('.modal-send-message').addClass('hidden');
                    window.location.href = '{{ route('catalog.message_sent', $partner->partner_link) }}';
                },
