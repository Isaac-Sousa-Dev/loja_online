<x-app-layout>
    ...
    <div class="flex justify-center">
        ...
        <div class="md:flex md:max-w-[1200px] flex-col w-full ml-2 mr-2">
            ...
            <div class="flex justify-between md:items-center flex-col md:flex-row mt-4">
                <h2 class="font-semibold text-3xl px-2.5 text-gray-800 leading-tight">
                    {{ $product->name }}
                </h2>
            </div>

            <div class="p-2.5 md:flex gap-2">

                <form class="md:w-[70%]" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="rounded-xl shadow-md pt-3 pb-2 mb-4 flex flex-col bg-white px-3">
                        ...
                        <div class="md:flex mb-0 gap-3">
                            <div class="w-full mb-3">
                                ...
                                <div class="w-full ">
                                    <select id="brand_id" class="select-brands border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-xl shadow-xs block w-full">        
                            ...
                            <div class="w-full mb-3">
                                ...
                                <div class="w-full ">
                                    <select id="model_id" class="select-models border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-xl shadow-xs block w-full">        
                                        @foreach($models as $model)
                                            <option @selected($model->id == $product->model_id) value="{{$model->id}}">{{$model->name}}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div>
                            <div class="w-full mb-3">
                                ...
                                <x-text-input id="name" class="required" placeholder="Posche Cayenne" name="name" type="text" value="{{$product->name}}" autofocus autocomplete="name" />
                            </div>
                        </div>

                        <div class="md:flex mb-0">
                            <div class="w-full mb-3">
                                <x-input-label for="description" :value="__('Sobre este veículo *')" />
                                <x-textarea id="description" class="required" placeholder="Descreva algo..." name="description" type="text" autofocus autocomplete="name" >{{$product->description}}</x-textarea>
                            </div>
                        </div>
                        ...
                        <div class="py-1 md:flex md:flex-row items-center gap-1 space-y-1 md:space-y-0">
                            <div class="grid grid-cols-2 gap-1 md:w-1/2">
                                <label for="" class="w-full">
                                    <div class="OpenModalImagesProducts relative bg-blue-50 w-full h-32 cursor-pointer rounded-lg border-dashed border-2 border-sky-500 flex flex-col px-0">
                                        <div class="font-semibold text-xs mt-1 flex z-10 ml-2">
                                            <div class="bg-gray-200 border-1 pb-[2px] border-gray-500 flex items-center px-1 rounded-full gap-1">
                                                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M259.3 17.8L194 150.2 47.9 171.5c-26.2 3.8-36.7 36.1-17.7 54.6l105.7 103-25 145.5c-4.5 26.3 23.2 46 46.4 33.7L288 439.6l130.7 68.7c23.2 12.2 50.9-7.4 46.4-33.7l-25-145.5 105.7-103c19-18.5 8.5-50.8-17.7-54.6L382 150.2 316.7 17.8c-11.7-23.6-45.6-23.9-57.4 0z"/></svg>
                                                principal
                                            </div>
                                        </div>
                                        <div class="h-2/3 flex flex-col justify-center items-center">
                                            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M352 240v32c0 6.6-5.4 12-12 12h-88v88c0 6.6-5.4 12-12 12h-32c-6.6 0-12-5.4-12-12v-88h-88c-6.6 0-12-5.4-12-12v-32c0-6.6 5.4-12 12-12h88v-88c0-6.6 5.4-12 12-12h32c6.6 0 12 5.4 12 12v88h88c6.6 0 12 5.4 12 12zm96-160v352c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V80c0-26.5 21.5-48 48-48h352c26.5 0 48 21.5 48 48zm-48 346V86c0-3.3-2.7-6-6-6H54c-3.3 0-6 2.7-6 6v340c0 3.3 2.7 6 6 6h340c3.3 0 6-2.7 6-6z"/></svg>
                                            <div class="leading-3 mt-2 text-center text-xs font-semibold px-2">
                                                Adicione a melhor foto do veículo
                                            </div>
                                        </div>
                                        <img style="display: none" src="" class="previewsImagesInProductPage object-cover h-full w-full absolute rounded-lg" alt="">
                                    </div>
                                </label>
    
                                <label for="" class="w-full">
                                    <div class="OpenModalImagesProducts relative bg-blue-50 w-full h-32 cursor-pointer rounded-lg border-dashed border-2 border-sky-500 flex flex-col justify-center items-center">
                                        <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M352 240v32c0 6.6-5.4 12-12 12h-88v88c0 6.6-5.4 12-12 12h-32c-6.6 0-12-5.4-12-12v-88h-88c-6.6 0-12-5.4-12-12v-32c0-6.6 5.4-12 12-12h88v-88c0-6.6 5.4-12 12-12h32c6.6 0 12 5.4 12 12v88h88c6.6 0 12 5.4 12 12zm96-160v352c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V80c0-26.5 21.5-48 48-48h352c26.5 0 48 21.5 48 48zm-48 346V86c0-3.3-2.7-6-6-6H54c-3.3 0-6 2.7-6 6v340c0 3.3 2.7 6 6 6h340c3.3 0 6-2.7 6-6z"/></svg>
                                        <div class="leading-3 mt-2 text-center text-xs font-semibold px-2">
                                            Suba o veículo de frente
                                        </div>
                                        <img style="display: none" src="" alt="" class="previewsImagesInProductPage object-cover h-full w-full absolute rounded-lg">
                                    </div>
                                </label>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-1 md:w-1/2">
                                ...
                                <label for="" class="w-full">
                                    <div class="OpenModalImagesProducts relative opacity-85 bg-blue-50 w-full h-32 cursor-pointer rounded-lg border-dashed border-2 border-sky-500 flex flex-col justify-center items-center">
                                        <div id="fourTextImageProductAfter" class="flex items-center justify-center text-center flex-col">
                                            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M352 240v32c0 6.6-5.4 12-12 12h-88v88c0 6.6-5.4 12-12 12h-32c-6.6 0-12-5.4-12-12v-88h-88c-6.6 0-12-5.4-12-12v-32c0-6.6 5.4-12 12-12h88v-88c0-6.6 5.4-12 12-12h32c6.6 0 12 5.4 12 12v88h88c6.6 0 12 5.4 12 12zm96-160v352c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V80c0-26.5 21.5-48 48-48h352c26.5 0 48 21.5 48 48zm-48 346V86c0-3.3-2.7-6-6-6H54c-3.3 0-6 2.7-6 6v340c0 3.3 2.7 6 6 6h340c3.3 0 6-2.7 6-6z"/></svg>
                                            <div  class="leading-3 mt-2 text-center text-xs font-semibold px-2">
                                                Mostre a parte interna
                                            </div>
                                        </div>

                                        <div id="fourTextImageProductBefore" style="display: none" class="leading-3 z-10 mt-2 text-center text-2xl font-semibold px-2">
                                            + <span id="quantityImagesProductAvailable"></span>
                                        </div>
                                        <img src="" style="display: none" id="previewSecondImage" alt="" class="previewsImagesInProductPage object-cover h-full w-full absolute opacity-50 rounded-lg">
                                    </div>
                                </label>
                            </div>
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="w-full flex gap-2">
                                <div class="w-1/2">
                                    ...
                                    <x-text-input id="price" class="price-mask required" placeholder="R$ 0.00" name="price" type="text" value="{{$product->price}}"  autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('price')" />
                                </div>

                                <div class="w-1/2">
                                    <x-input-label for="price_promotional" :value="__('Preço promocional')" />
                                    <x-text-input id="price_promotional" class="price-mask" placeholder="R$ 0.00" name="price_promotional" value="{{$product->price_promotional}}" type="text" autofocus autocomplete="name" />
                            ...
                            <div class="w-full flex gap-2">
                                <div class="w-1/2">
                                    ...
                                    <x-text-input id="cost" class="price-mask" placeholder="R$ 0.00" name="cost" value="{{$product->cost}}" type="text" autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('cost')" />
                                </div>

                                <div class="w-1/2">
                                    <x-input-label for="profit" :value="__('Margem de lucro')" />
                                    <x-text-input disabled id="profit" data-profit="{{$product}}" placeholder="--" value="{{$product->profit}}" name="profit" type="text" class="bg-gray-200"  autofocus autocomplete="name" />
                    ...
                    <div class="rounded-xl shadow-md pt-3 mb-4 flex flex-col pb-3 bg-white px-3">
                        <div class="text-lg font-semibold text-gray-800 mb-3">
                            ...
                            <div class="flex gap-1">
                                ...
                                <span class="text-blue-700 text-xs">As propriedades aparecem na página do veículo.</span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="w-full flex flex-wrap gap-2">
                                ...
                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="reindeer" :value="__('Renavam')" />
                                    <x-text-input id="reindeer" class="renavam-mask" value="{{$product->properties->reindeer}}" placeholder="00000000000" name="reindeer" type="text" />
                                    <x-input-error class="mt-2" :messages="$errors->get('reindeer')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="chassis" :value="__('Chassi')" />
                                    <x-text-input id="chassis" class="chassi-mask" value="{{$product->properties->chassis}}" placeholder="0000000000000XXXX" name="chassis" type="text" />
                                    <x-input-error class="mt-2" :messages="$errors->get('chassis')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="engine" :value="__('Motor')" />
                                    <x-text-input id="engine" class="engine-power-mask" value="{{$product->properties->engine}}" placeholder="2.0" name="engine" type="text" />
                                    <x-input-error class="mt-2" :messages="$errors->get('engine')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="year_of_manufacture" :value="__('Ano')" />
                                    <x-text-input id="year_of_manufacture" value="{{$product->properties->year_of_manufacture}}" class="year-manufacturer-mask" placeholder="2021/2022" name="year_of_manufacture" type="text"  autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('year_of_manufacture')" />
                                </div>
 
                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="fuel" :value="__('Combustível')" />
                                    <x-text-input id="fuel" value="{{$product->properties->fuel}}" placeholder="Flex" name="fuel" type="text"  autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('fuel')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="license_plate" :value="__('Placa')" />
                                    <x-text-input id="license_plate" class="license-plate-mask" value="{{$product->properties->license_plate}}" placeholder="PMO3R7X" name="license_plate" type="text" autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('license_plate')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="miliage" :value="__('KM')" />
                                    <x-text-input id="miliage" class="miliage-mask" value="{{$product->properties->miliage}}" placeholder="116.143" name="miliage" type="text" autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('miliage')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="exchange" :value="__('Câmbio')" />
                                    <x-text-input id="exchange" value="{{$product->properties->exchange}}" placeholder="Manual" name="exchange" type="text" autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('exchange')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="bodywork" :value="__('Carroceria')" />
                                    <x-text-input id="bodywork" value="{{$product->properties->bodywork}}" placeholder="Picapes" name="bodywork" type="text" autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('bodywork')" />
                                </div>

                                <div class="w-[48.5%] md:w-[32.5%]">
                                    <x-input-label for="color" :value="__('Cor')" />
                                    <x-text-input id="color" value="{{$product->properties->color}}" placeholder="Vermelho" name="color" type="text" autofocus autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('color')" />
                                </div>


                                <div class="w-[48.5%] md:w-[32.5%]" >
                                    <x-input-label for="accept_exchange" :value="__('Aceita troca')" />
                                    {{-- {{$product->accept_exchange}} --}}
                                    <select id="accept_exchange" placeholder="Não" name="accept_exchange" type="text" class="border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-md h-9 shadow-xs w-full">
                                        <option @selected($product->properties->accept_exchange == 0) value="0">Não</option>
                                        <option @selected($product->properties->accept_exchange == 1) value="1">Sim</option>
                                    </select>
                                    <x-input-error class="mt-2" :messages="$errors->get('accept_exchange')" />
                                </div>

                                <div class="w-full md:w-[32.5%]">
                                    <x-input-label for="review_done" :value="__('Revisão feita pela loja')" />
                                    <select id="review_done" placeholder="Não" name="review_done" type="text" class="border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-md h-9 shadow-xs w-full">
                                        <option @selected($product->properties->review_done == 1) value="1">Sim</option>
                                        <option @selected($product->properties->review_done == 0) value="0">Não</option>
                                    </select>
                                    <x-input-error class="mt-2" :messages="$errors->get('review_done')" />
                                </div>
                ...
                <div class="md:w-[30%]">
                    <div class="text-lg h-[31.5%] text-gray-800 rounded-xl shadow-md pt-3 pb-2 mb-4 flex flex-col bg-white px-3">
                        <div class="mb-3 font-semibold">
                            Documentação
                        </div>
                        <div class="flex flex-col gap-3">
                            <div class="w-full">
                                <x-input-label for="type" :value="__('Classificação')" />
                                <select id="type" placeholder="Não" name="type" type="text" class="border-gray-300 pt-1.5 focus:border-indigo-300 focus:ring-indigo-300 rounded-md h-9 shadow-xs w-full">
                                    <option @selected($product->type == 'owner') value="owner">Próprio</option>
                                    <option @selected($product->type == 'consigned') value="consigned">Consignado</option>
                                </select>
                                <x-input-error class="mt-2" :messages="$errors->get('type')" />
                            </div>

                            <x-file-upload label="CRLV" id="crlv" :file_url="$product->crlv" />
                            <x-file-upload label="DUT" id="dut" :file_url="$product->dut" />
                            <x-file-upload label="Nota fiscal" id="purchase_invoice" :file_url="$product->invoice" />
            ...
            <div class="w-full justify-end flex mb-4 gap-2">
                <div class="flex w-full md:w-2/6 justify-between gap-3 py-2 px-3 bg-white rounded-xl">
                    ...
                    <x-primary-button id="btnUpdateDataProduct">{{ __('Salvar') }}</x-primary-button>
                </div>
            </div>
        </div>
    </div>
...
<div 
    ...
    <div class="header px-4 py-1">
        <!-- Botão Fechar -->
        <button
           id="close-images-product-slider"
           class="mt-4 px-2 py-2 bg-white rounded-lg border-1 hover:border-gray-500 text-white hover:bg-gray-500"
       >
            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M31.7 239l136-136c9.4-9.4 24.6-9.4 33.9 0l22.6 22.6c9.4 9.4 9.4 24.6 0 33.9L127.9 256l96.4 96.4c9.4 9.4 9.4 24.6 0 33.9L201.7 409c-9.4 9.4-24.6 9.4-33.9 0l-136-136c-9.5-9.4-9.5-24.6-.1-34z"/></svg>
       </button>
    </div>
    <div class="p-4">
        ...
        <div id="info-max-images" style="display: none" class="bg-yellow-50 p-2 rounded-lg mb-2 text-center border-1 border-yellow-300 text-sm font-semibold text-yellow-700">
        ...
        <div class="gallery-slide overflow-y-auto md:h-[600px]">
            ...
            <form id="image-upload-form">
                ...
                <!-- PREVIEW DAS IMAGENS -->
                <div id="previewImages" class="justify-between grid grid-cols-2 flex-wrap space-x-1 md:mb-44 mt-4"></div>    
            </form> 
        </div>
    ...
    <div class="fixed bottom-0 w-full px-4 py-2">
        <label for="label-for-images-product" class="label-for-images-product-edit w-full cursor-pointer">
            <div class="OpenModalImagesProducts bg-blue-50 w-full h-14 cursor-pointer rounded-lg border-dashed border-2 border-sky-500 flex flex-col px-1">
...
<script>
        ...
        imagesArray.forEach(image => {
            $('#previewImages').append(`
                <div class="rounded-md relative h-32 static mb-2" data-divimageid="${image.index}">
                    ...
                    <img src="${image.url}" class="w-full h-32 shadow-md rounded-md mb-2 object-cover">
                </div>
            `); 
        })
    ...
    $( document ).ready(function() {
        ...
        if(imagesArray[0]) {
            previewsImagesInProductPage[0].src = imagesArray[0].url
            previewsImagesInProductPage[0].style.display = "block"
        }
        if(imagesArray[1]) {
            previewsImagesInProductPage[1].src = imagesArray[1].url
            previewsImagesInProductPage[1].style.display = "block"
        }
        if(imagesArray[2]) {
            previewsImagesInProductPage[2].src = imagesArray[2].url
            previewsImagesInProductPage[2].style.display = "block"
        }
        if(imagesArray[3]) {
            previewsImagesInProductPage[3].src = imagesArray[3].url
    ...
    $(inputImagesProduct).change(function(e) {
        ...
        for (let i = 0; i < filesToProcess; i++) {
            ...
            reader.onload = function(e) {
                ...
                $('#previewImages').append(`
                    <div class="rounded-md h-32 relative static mb-2" id="teste" data-indexnewfile="${fileFormatted.indexNewFile}" data-divimageid="${fileFormatted.index}">
                        <div class="absolute">
                            <button type="button" class="btn-remove-image-product text-xs bg-gray-200 text-white px-2 py-1 rounded-md mt-2 ml-2">
                                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M268 416h24a12 12 0 0 0 12-12V188a12 12 0 0 0 -12-12h-24a12 12 0 0 0 -12 12v216a12 12 0 0 0 12 12zM432 80h-82.4l-34-56.7A48 48 0 0 0 274.4 0H173.6a48 48 0 0 0 -41.2 23.3L98.4 80H16A16 16 0 0 0 0 96v16a16 16 0 0 0 16 16h16v336a48 48 0 0 0 48 48h288a48 48 0 0 0 48-48V128h16a16 16 0 0 0 16-16V96a16 16 0 0 0 -16-16zM171.8 50.9A6 6 0 0 1 177 48h94a6 6 0 0 1 5.2 2.9L293.6 80H154.4zM368 464H80V128h288zm-212-48h24a12 12 0 0 0 12-12V188a12 12 0 0 0 -12-12h-24a12 12 0 0 0 -12 12v216a12 12 0 0 0 12 12z"/></svg>
                            </button>
                        </div>
                        <img src="${e.target.result}" id="image-${i}" class="w-full object-cover object-contain h-32 shadow-md rounded-md mb-2">
                    </div>
                `);
            ...
            console.log(imagesArray, 'Minhas imagens no change');
        ...
        if (files.length > remainingSlots) {
            ...
            infoMaxImages[0].innerText = `Você tentou carregar ${files.length} imagens, mas só ${remainingSlots} foram adicionadas. O limite máximo é de 8 imagens.`;
    ...
    $('#previewImages').on('click', '.btn-remove-image-product', function(e) {
        ...
        console.log(newsFiles, 'NewsFiles')
        ...
        if(imageId == undefined) {
            ...
            // newsFiles = newsFiles.filter(file => file.id !== imageId);
        }

        if(imagesArray.length < 8) {
            let infoMaxImages = $('#info-max-images');
            infoMaxImages[0].style.display = 'none';
            labelForImagesProductEdit[0].style.display = 'block';
        }

        // Adicione o ID da imagem ao array de exclusão
    ...
    const sortable = new Sortable(previewImages, {
        ...
        onSort: function () {
            ...
            $('#previewImages > div').each(function (e, index) {
                const divImageId = $(this).data('divimageid'); // Pegue o nome da imagem do atributo `data-name`
                console.log(divImageId, 'DivImageId')
                const imageData = imagesArray.find(image => image.index === divImageId); // Encontre a imagem correspondente no array
                if (imageData) {
    ...
    $('#btnUpdateDataProduct').click(function(e) {
        ...
        // Só adiciona se tiver arquivo novo no input
        console.log($('#crlv')[0].files, 'Files CRLV')
        if ($('#crlv')[0].files.length > 0) {
            formData.append('crlv', $('#crlv')[0].files[0]);
        }
        if ($('#dut')[0].files.length > 0) {
            formData.append('dut', $('#dut')[0].files[0]);
        }
        if ($('#purchase_invoice')[0].files.length > 0) {
            formData.append('invoice', $('#purchase_invoice')[0].files[0]);
        }
