...
<div class="md:flex md:max-w-[1200px] flex-col w-full px-3 md:px-2">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-gray-500 mt-4 mb-2 px-1" aria-label="Breadcrumb">
        ...
        <i class="fa-solid fa-chevron-right text-[10px] text-gray-400" aria-hidden="true"></i>
        <span class="font-semibold text-gray-700 truncate max-w-[200px]">Editar</span>
    </nav>
    ...
    <div class="flex items-center gap-3 mt-1 mb-5 px-1">
        ...
        <h2 class="font-display font-semibold text-2xl text-[#33363B] leading-tight truncate">{{ $product->name }}</h2>
    </div>

    {{-- TABS --}}
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 mb-4 overflow-hidden">

        <div class="flex border-b border-gray-100 overflow-x-auto overflow-y-hidden scrollbar-none" role="tablist" aria-label="Etapas da edição" style="-webkit-overflow-scrolling:touch; scrollbar-width:none; -ms-overflow-style:none;">
        ...
        <div id="mainTab-geral" role="tabpanel" aria-labelledby="mainTabBtn-geral" class="p-3 md:p-5">
            <form id="productForm" action="{{ route('products.wizard.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="flex-1 flex flex-col gap-5">
                        <div>
                            ...
                            <div class="mb-3">
                                ...
                                <x-text-input id="name" placeholder="Ex: Camisa Polo Azul" name="name" type="text" :value="$product->name" class="mt-1 w-full step1-required" />
                            </div>
                            <div>
                                <x-input-label for="description" :value="__('Descrição *')" />
                                <x-textarea id="description" class="step1-required mt-1" placeholder="Descreva o produto..." name="description">{{ $product->description }}</x-textarea>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-gray-100 pt-4">
                            ...
                            <div class="grid grid-cols-1 gap-3 mb-3 md:grid-cols-2 xl:grid-cols-3">
                                <div><x-input-label for="price" :value="__('Varejo *')" /><x-text-input id="price" placeholder="0,00" name="price" type="text" :value="$product->price" class="price-mask step1-required mt-1 w-full" /></div>
                                ...
                                @foreach ($wholesaleLevels as $level)
                                    <div>
                                        ...
                                        <x-text-input
                                            ...
                                            type="text"
                                            :value="$productWholesalePrices[$level['id']] ?? null"
                        ...
                        <div class="bg-gray-50 rounded-xl border border-gray-100 p-4">
                            ...
                            <div>
                                ...
                                <select id="gender" name="gender" class="border-gray-300 focus:border-indigo-300 focus:ring-indigo-300 rounded-xl h-9 shadow-xs w-full mt-1 text-sm">
                                    <option value="" @selected($genderSelect === '')>Selecione</option>
                                    <option value="M" @selected($genderSelect === 'M')>Masculino</option>
                                    <option value="F" @selected($genderSelect === 'F')>Feminino</option>
        ...
        <div id="mainTab-atributos" role="tabpanel" aria-labelledby="mainTabBtn-atributos" class="hidden p-2 md:p-5">
            <x-product-variants-manager
                ...
                :existingVariants="$existingVariantsJson"
            />
        </div>
